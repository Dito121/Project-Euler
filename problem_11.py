        ...
        self.max_diagonal = max(
            ...
            sum(
                [
                    ...
                    self.grid[self.p[0] + 2][self.p[1] + 2],
                    self.grid[self.p[0] + 3][self.p[1] + 3],
                ]
            ),
        )

    def solve(self):
        while self.p[0] < self.rows:
            while self.p[1] < self.cols:
                if self.p[1] < self.cols - 4:
                    self.horizontal()
                    if self.p[0] < self.rows - 4:
                        self.diagonal()
